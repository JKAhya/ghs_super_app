...
<main class="bg-base-100 flex h-full w-full flex-col items-center justify-center p-4 font-bold">
	<div class="pt-5 pb-11">
		<ul class="menu menu-horizontal bg-base-200 rounded-2xl p-4 px-10 font-normal shadow-lg">
	...
	<section
		class="border-primary h-screen min-h-9/12 w-full max-w-5xl overflow-hidden rounded-lg border-2 shadow-lg focus:ring-4 focus:ring-blue-500/50 focus:outline-none"
	...
	>
		<svg
			...
			class="h-full w-full cursor-grab bg-stone-50"
			role="img"
			aria-label="층별 약도 상세"
		>
			<slot />
	...
	<div class="text-base-content mt-4 flex flex-wrap items-center justify-center gap-2">
		<div class="border-base-300 flex items-center rounded-lg border">
			<button
				on:click={() => pan('left')}
				class="btn btn-ghost rounded-l-md p-2"
				aria-label="왼쪽으로 이동">←</button
			>
			<div class="flex flex-col">
				<button on:click={() => pan('up')} class="btn btn-ghost p-2" aria-label="위로 이동"
					>↑</button
				>
				<button on:click={() => pan('down')} class="btn btn-ghost p-2" aria-label="아래로 이동"
					>↓</button
				>
			</div>
			<button
				on:click={() => pan('right')}
				class="btn btn-ghost rounded-r-md p-2"
				aria-label="오른쪽으로 이동">→</button
			>
		</div>
		<div class="border-base-300 flex items-center rounded-lg border">
			<button
				on:click={() => zoom('in')}
				class="btn btn-ghost rounded-l-md p-2 font-bold"
				aria-label="확대">+</button
			>
			<button on:click={() => zoom('out')} class="btn btn-ghost p-2 font-bold" aria-label="축소"
				>-</button
			>
			<button on:click={resetZoom} class="btn btn-ghost rounded-r-md p-2" aria-label="기본 보기"
				>⟲</button
			>
		</div>
	</div>
	...
	<div class=" mt-10 text-center">
		Powered by map.hya35.com<br />
		<div class="font-normal">
			이 기능은 Slat 시간에 기흥고등학교 김정현에 의해 <br />CC0 1.0 Universal(퍼블릭 도메인)
			라이선스로 제작 및 배포된 <wbr /> 기흥고등학교 약도 SVG 파일과 이것을 열람하기 위한 지도형 뷰어
			전체를 별도의 수정 없이 탑제한것입니다.
		</div>
	</div>
</main>
