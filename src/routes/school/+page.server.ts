...
export async function load({ fetch }) {
  ...
  try {
    ...
    const mealContentElement = root.querySelector('#container > div.MC_wrap2 > div > div.MC_box4.widgEdit > div > div > ul > li > dl > dd');
    ...
    const rawLines = mealContentElement.textContent.trim().split('\n').filter(line => line.trim() !== '');
    ...
    for (const line of rawLines) {
      ...
      if (allergyOnlyMatch) {
        ...
        if (lastMenuIndex !== -1) {
          ...
          const codes = allergyOnlyMatch[1].split('.').map(code => code.trim()).filter(code => code !== '');
          parsedContent[lastMenuIndex].allergyCodes.push(...codes);
          // 중복 제거
          parsedContent[lastMenuIndex].allergyCodes = [...new Set(parsedContent[lastMenuIndex].allergyCodes)];
        }
      } else {
        ...
        if (directAllergyMatch) {
          ...
          allergyCodes = directAllergyMatch[2].split('.').map(code => code.trim()).filter(code => code !== '');
        } 
