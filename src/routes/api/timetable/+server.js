...
export async function GET(event) {
    ...
    try {
        ...
        if (gradeParam) {
            ...
            if (isNaN(targetGrade) || targetGrade < 1 || targetGrade > 3) { // 최대 학년은 3학년으로 가정
                return new Response(JSON.stringify({ error: '유효하지 않은 학년(grade) 파라미터입니다. 1에서 3 사이의 숫자를 입력하세요.' }), {
                    status: 400,
        ...
        if (classParam) {
            ...
            if (isNaN(targetClass) || targetClass < 1) { // 반은 1 이상
                return new Response(JSON.stringify({ error: '유효하지 않은 반(class) 파라미터입니다. 1 이상의 숫자를 입력하세요.' }), {
                    status: 400,
                    headers: { 'Content-Type': 'application/json' }
                });
            }
        ...
        if (targetGrade !== null && targetClass !== null) {
            ...
            if (fullTimetableData[targetGrade] && fullTimetableData[targetGrade][targetClass]) {
                ...
                return new Response(JSON.stringify({ error: `학년 ${targetGrade}반 ${targetClass}의 시간표를 찾을 수 없습니다.` }), {
                    status: 404,
                    headers: { 'Content-Type': 'application/json' }
                });
            }
